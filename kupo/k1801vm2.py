...
class k1801bm1_processor_t(idaapi.processor_t):
    ...
    # IDA supports values up to 0o32 bits
    cnbits = 8
    ...
    reg_first_sreg = 0o16  # index of CS
    reg_last_sreg = 0o17   # index of DS
    ...
    instruc = [
        ...
        {'name': 'iot',   'feature': CF_CALL, 'cmt': 'Trap to 0o20'},
    ...
    def notify_emu(self, insn):
        ...
        if insn.itype == self.itype_emt and insn.Op1.value == 0o0376:
            create_byte(newEA, 2)
            newEA += 2
            add_cref(insn.ea, newEA, fl_F)
        elif (self.flow and
              not(insn.itype == self.itype_emt and insn.Op1.value == 0o0350)):
    ...
    def op_t_mem(self, insn, op, isload):
        """
        op.type:
            # case o_near:       // Jcc/ [jmp/call 0o37/67]
            # case o_mem:        // 0o37/67/77
            # case o_far:
        """
        if (op.type == o_far):
    ...
    def op_t_displ(self, insn, op, is_forced, isload):
        ...
        """
        set_immd(insn.ea)
        if (not(isload) and op.phrase == (0o060 + self.ireg_R0)
                and addr16(op) <= 1):
    ...
    def op_t_number(self, insn, op):
        ...
        if insn.itype == self.itype_emt and get_cmt(NULL, insn.ea, False) <= 0:
            tmp = insn
            tmpx = tmp.ops[op.n]
            if (tmpx.value >= 0o0374 and tmpx.value <= 0o0375):
                if (tmpx.value == 0o0375):
    ...
    def op_t_phrase(self, insn, op, isload):
        if (op.phrase & 7) == self.ireg_R0:
            if (not(isload) and op.phrase == (0o010 + self.ireg_R0)):
    ...
    def _out_o_mem(self, ctx, op):
        ...
        if not ctx.out_name_expr(op, segadr, addr16(op)):
            if op.type == o_far or addr16(op) < 0o0160000:
    ...
    def jmpoper(self, insn, op, nibble0):
        ...
        if op.type == o_mem and op.phrase != 0o077:
            op.type = o_near
        # FIXME ml & ovrtrans !!!
    ...
    def loadoper(self, insn, op, nibble):
        ...
        elif nibble in (0o037, 0o077, 0o067):
            op.type = o_mem
            op.offb = insn.size & 0xff
    ...
    def ana_nib2_007(self, insn, nibble0, nibble1):
        """
        Analyze nibble2 == 0o07
        """
        nib1swt = nibble1 >> 3
        if nib1swt == 6:  # CIS
            raise InvalidInsnError()
        elif nib1swt == 5:  # FIS
            if nibble1 != 0o050 or nibble0 >= 0o040:
                raise InvalidInsnError()
    ...
    def ana_nib2_000(self, insn, nibble0, nibble1):
        ...
        if 3 == nibble1:
            ...
            if nibble0 < 0o040:
                ...
                return
            v = nibble0 & 0o037
            if 0o000 == v: insn.itype = self.itype_nop
            elif 0o001 == v: insn.itype = self.itype_clc
            elif 0o002 == v: insn.itype = self.itype_clv
            elif 0o004 == v: insn.itype = self.itype_clz
            elif 0o010 == v: insn.itype = self.itype_cln
            elif 0o017 == v: insn.itype = self.itype_ccc
            elif 0o021 == v: insn.itype = self.itype_sec
            elif 0o022 == v: insn.itype = self.itype_sev
            elif 0o024 == v: insn.itype = self.itype_sez
            elif 0o030 == v: insn.itype = self.itype_sen
            elif 0o037 == v: insn.itype = self.itype_scc
            else:
            ...
            if nibble0 <= 7:
                ...
                return
            if nibble0 < 0o020 and (self.cpu_1801bm1 or self.cpu_1801bm2):
                # 1801BM1/BM2 specific: START and STEP
                if (nibble0 & 0o014) == 0o010:
                    insn.itype = self.itype_start
                else:  # elif (nibble0 & 0o014) == 0o014:
                    insn.itype = self.itype_step
                return
            elif self.cpu_1801bm2 and nibble0 <= 0o037:
    ...
    def oneoper(self, insn, nibble0, nibble1):
        ...
        insn.itype = onecmd[nibble1 - 0o050]

    def ana_nib2_010(self, insn, nibble0, nibble1):
        """
        Analyze nibble2 == 0o010
        """
        if nibble1 >= 0o070:
            raise InvalidInsnError()
        # nib1swt = nibble1 >> 3
        if nibble1 >= 0o064:
            ...
            insn.itype = mt1cmd[nibble1 - 0o064]
            self.loadoper(insn, insn.Op1, nibble0)
            return
        if nibble1 >= 0o050:
            set_bytecmd(insn)
        ...
        if nibble1 >= 0o040:
            ...
            insn.Op1.value = self._code & 0o0377
            if nibble1 >= 0o044:
    ...
    def notify_ana(self, insn):
        ...
        nibble0 = (self._code & 0o077)
        nibble1 = (self._code >> 6) & 0o077
        nibble2 = (self._code >> 0o12) & 0o017
        try:
            if nibble2 == 0o017:
    ...
    def handle_byte_op(self, op):
        ...
        """
        if ((op.type == o_mem and op.phrase != 0o077) or
                (op.type == o_displ and (op.phrase & 0o070) == 0o060)):
            op.dtype = dt_byte
    ...
    def init_instructions(self):
        ...
        self.bm2_cmd.update({
            0o020: self.itype_rsel, 0o021: self.itype_mfus,
            0o022: self.itype_rcpc, 0o024: self.itype_rcps,
            0o031: self.itype_mtus,
            0o032: self.itype_wcpc, 0o034: self.itype_wcps
        })
        self.bm1_cmd.update({
            0o012: self.itype_start,
            0o016: self.itype_step
        })
    ...
    def __init__(self):
        ...
        self.init_registers()
        self._code = None  # current insn code (0o16bit word) in ana()
        self.flow = None
        self.emuFlg = False
        self.emuR0 = 0xffff
        self.emuR0data = 0xffff
